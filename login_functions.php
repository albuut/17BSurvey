...
if (isset($_POST['manage_user_btn'])){
    header('location: manage_user.php');
}
...
if (isset($_POST['create_btn'])) {
    header('location: survey_setting.php');
}
...
if (isset($_POST['confirm_delete_btn'])){
    ...
    if(strtolower($_POST['confirm']) == "confirm"){
        ...
        $query_delete_user = "DELETE FROM anguyen_entity_user WHERE id_user='$user_id'";
        $query_delete_user_answer = "DELETE FROM anguyen_xref_answer_user WHERE user_id='$user_id'";        
        $query_delete_question_user = "DELETE FROM anguyen_xref_question_user WHERE user_id='$user_id'";
        
        $query_answer_id = "SELECT * FROM anguyen_xref_answer_user WHERE user_id='$user_id'";
        $result_delete_user_answer = mysqli_query($db,$query_delete_user_answer);    
        
        while($row = mysqli_fetch_array($result_delete_user_answer)){
            $answer_id = $row['answer_id'];
            $query_delete_answer_question = "DELETE from anguyen_xref_question_answer WHERE answer_id='$answer_id'";
            mysqli_query($db,$query_delete_answer_question);
        }
        
        mysqli_query($db,$query_delete_user);
        mysqli_query($db,$query_delete_user_answer);
        mysqli_query($db,$query_delete_question_user);
...
if(isset($_POST['delete_user_btn'])){
    ...
    if($user_id == $current_id){
        ...
        $query_name = "SELECT * FROM anguyen_entity_user WHERE id_user='$user_id'";
        $result_name = mysqli_query($db,$query_name);
        $data_name = mysqli_fetch_assoc($result_name);
        $_SESSION['delete_username'] = $data_name['username'];
...
if (isset($_POST['manage_confirm_btn'])) {
    ...
    $query_account_info = "SELECT * FROM anguyen_entity_user WHERE id_user='$user_id'";
    ...
    if(strcasecmp($username,$account_data['username']) == 0){
        $query = "SELECT * FROM anguyen_entity_user WHERE username='$username'";
    ...
    if(strcasecmp($email,$account_data['email']) == 0){
        $query = "SELECT * FROM anguyen_entity_user WHERE email='$email'";
        $dupe = mysqli_query($db, $query);
    ...
    if(count($errors) == 0){
        $update_username = "UPDATE anguyen_entity_user SET username='$username' WHERE id_user='$user_id'";
        mysqli_query($db,$update_username);
        $update_email = "UPDATE anguyen_entity_user SET email='$email' WHERE id_user='$user_id'";
        mysqli_query($db,$update_email);

        if(!empty($new_pass1)){
            $new_password = md5($new_pass1);
            $update_password = "UPDATE anguyen_entity_user SET password='$new_password' WHERE id_user='$user_id'";
            mysqli_query($db,$update_password);
        }
        
        if($account_data['user_type'] != $_POST['user_type']){
            $user_type = $_POST['user_type'];
            $update_type = "UPDATE anguyen_entity_user SET user_type='$user_type' WHERE id_user='$user_id'";
...
function register(){
	...
	if (count($errors) == 0) {
		...
		if (isset($_POST['user_type']) && isset($_SESSION['user'])) {
			...
			$query = "INSERT INTO `anguyen_entity_user`(`username`, `email`, `user_type`, `password`) 
...
function login(){
	...
	if (count($errors) == 0) {
		...
		$query = "SELECT * FROM anguyen_entity_user WHERE username='$username' AND password='$password' LIMIT 1";
